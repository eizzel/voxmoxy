<?php
/**
 * CGFileHelper class file.
 * This is a modified version of {@link CFileHelper} which should be used when working under Google App Engine
...
/**
 * CGFileHelper provides a set of helper methods for common file system operations.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class CGFileHelper
{
    ...
    public static function copyDirectory($src,$dst,$options=array())
    {
        ...
        // not need to create directories as App Engine do not uses them
        // and will create the concept of directory on the fly
//        if(!is_dir($dst))
//            self::mkdir($dst,$options,true);
    ...
    protected static function copyDirectoryRecursive($src,$dst,$base,$fileTypes,$exclude,$level,$options)
    {
        ...
        while(($file=readdir($folder))!==false)
        {
            ...
            if(self::validatePath($base,$file,$isFile,$fileTypes,$exclude))
            {
                if($isFile)
                {
                    if (self::isGooglePath($dst)) {
                        // Google Cloud Storage
                        self::gCopy($path, $dst.DIRECTORY_SEPARATOR.$file);
                    } else {
                        // local file system
    ...
    {
        // No need to create directories under Google App Engine (gs://)
        if (self::isGooglePath($dst)) {
            return true;
        }
    ...
    /**
     * Checks if given path is Google Cloud Storage path
     *
     * @param string $path
     * @return bool
     */
    private static function isGooglePath($path)
    {
        if (preg_match('@^gs://@', $path)) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * Copy file from local file system to Google Cloud Storage system
     *
     * @param string $src
